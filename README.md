### Laboratorium 2 (24.02.2024)
Temat: Grafika 2D z użyciem HTML Canvas

### Polecenie: 

1. Narysować obraz zgodnie z wariantem zadania (używając zarówno standardowe jak i niestandardowe funkcje rysowania).

2. W Plik Lab2Ex2.html program domyślnie rysuje szereg kwadratów, stworzyć narzędzia pozwalające na wykonywanie czynności:
- "czyszczenie" canvasu - Clear button:
- dodanie jednego nowego koloru do elementu <select>. Implementować nowy kolor przez funkcję doMouseMove.
-  opracowanie nowego narzędzia - rysowania szeregu wielokątów (zgodnie z wariantem zadania). Opcja ma być dostępna przez nowy element <select>

### Wykonał
- Radosław Walczak
- Informatyka I stopień,
- niestacjonarne,
- 4 semestr,
- Gr.2A/3
